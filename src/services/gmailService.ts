...
/**
 * Checks via the Gmail API whether the lead has replied to the thread since the last send.
 */
export async function checkThreadForLeadReply(
  ...
  const { messages } = await getGmailThread(token, threadId, userEmail);
  ...
  // If lastSentDate is provided, verify the reply came after or at that date
  ...
  if (lastSentDate) {
    ...
    // Give a 60 second margin for clock discrepancies
...
/**
 * Sends a stage email to a lead using Gmail API.
 * If lead has an existing threadId, replies within the same thread with In-Reply-To and References.
 * Returns the Gmail message ID and threadId.
 */
export async function sendStageEmail(
  ...
  if (lead.threadId) {
    try {
      const threadData = await getGmailThread(token, lead.threadId, userEmail);
